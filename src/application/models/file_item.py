...
class FileItem():
    ...
    def to_string(self):
        return f'Process Date = [{self.effective_date}] Client Account = [{self.client_account}] External Refrence = [{self.external_refrence}] Company Name = [{self.company_name}] Amount = [{self.amount}] Status = [{self.status.value}]'
